...
% v2: allow for neff<>1 
%           - Note: assumed neff=ncladding i.e. ignores the (small) difference between guided mode and cladding propagation
%           - adapt target beam definition (plane wave and Gauss)
%           - adapt grating period
...
% define target beam (scalar), propagate to grating plane
    ...
    if (0==0)   % plane wave
        ...
        ntar = [0 1 1];   % direction of propagation
        ntar = ntar/norm(ntar);

        k2x = neff*k0*ntar(1);     % v2: neff*k0 (=beta), not k0
        k2y = neff*k0*ntar(2);
        k2z = neff*k0*ntar(3);
    ...
    if (1==0)   % tilted Gaussian
        ...
        E2 = E20 * w2./w2z .* exp(-(xxr./w2z).^2) .* exp(1i*(neff*k0*zzr+neff*k0*xxr.^2.*Rzi/2-eta));   % v2: neff
    end
...
lamgrat0 = lamgrat0 * lam/neff;     % v2: neff
...
% at each position, calculate local target propagation direction, local grating period
    ...
    kz = sqrt(neff^2*k0^2-kx.^2-ky.^2);     % v2: neff

    [lamgrat,alphagrat,alphatilt] = grating_angles_3D_f2(kx,ky,kz);
    lamgrat = lamgrat * lam/neff;     % v2: neff
...
% using ideal grating tilt at every point
%[al1,Ex1,Ey1,Ez1] = scatteringrate3_f(lamgrat,alphagrat,alphatilt,ky,pol);
% using grating tilt angle of centre everywhere
[al1,Ex1,Ey1,Ez1] = scatteringrate3_f(lamgrat,alphagrat,alphatilt0,ky,pol);
...
if (selpump==3)    % case 3: optimised pump at input, choose max dng for 1/2 pump depletion
    ...
    dngbar = dngbar_max*sqrt(0.99);     % 0.01, 0.99
...
%alphagrat0
phi = angle(E2.*exp(-1i*neff*k0*(ntar(1)*xxplot+ntar(2)*yyplot+ntar(3)*zzplot)));   % v2: neff

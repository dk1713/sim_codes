...
% v3: vary the grating strength dng and keep pump amplitudes constant to achieve target polarisation 
%     (instead of keeping grating contrast dng=1 constant and very the pump amplitude for v1 and v2)
% v3a: allow for neff<>1 
...
neff = 2;               % effective index of fundamental mode
...
% figure(2)
% subplot(231)
% pcolor(xx,yy,lamgrat1/(lam/neff)), shading flat, colorbar     % vXa: neff
% xlabel('x'), ylabel('y'), title('Grating 1 period \Lambda/(\lambda/neff)')
% subplot(232)
...
% subplot(234)
% pcolor(xx,yy,lamgrat2/(lam/neff)), shading flat, colorbar     % vXa: neff
% xlabel('x'), ylabel('y'), title('Grating 2 period \Lambda/(\lambda/neff)')
% subplot(235)
...
A2 = (-a2.*ptar1+a1.*ptar2) ./ (a1.*b2-a2.*b1);
    ...
    % now change this from varying pump amplitudes to varying dng
...
% subplot(222)
% pcolor(xx,yy,dng2), shading flat, colorbar
% xlabel('x'), ylabel('y'), title(['Pol. ' num2str(polout) ': Grating 2 strength dng_2'])
% subplot(223)
...
alphagrat2p = alphagrat2 - pi/2 + pi*upw1;
    ...
    % calculate scattering rate (for dng=1) and scattering direction
[al1cross,~,~,~] = scatteringrate3_f(lamgrat2,alphagrat2p,alphatilt2,ky,pol);

n1crossx = ( neff*k0-2*pi./lamgrat2.*cos(alphagrat2p) ) / (k0*neff);     % vXa: neff
n1crossy = ( -2*pi./lamgrat2.*sin(alphagrat2p) ) / (k0*neff);     % vXa: neff
...
n2crossy = ( neff*k0-2*pi./lamgrat1.*cos(alphagrat1p) ) / (k0*neff);   % in pump 1 (true) coordinate system     % vXa: neff
n2crossx = ( +2*pi./lamgrat1.*sin(alphagrat1p) ) / (k0*neff);     % vXa: neff

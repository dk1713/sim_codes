...
% rotated frame
phis    = linspace(-30,30,13)*pi/180;
eff_s   = zeros(size(phis));
eff_p   = zeros(size(phis));

for ite = 1:length(phis)
    phi = phis(ite);
    ...
    %%
    % figure(4); clf;
    % ax1 = axes; pcolor(x*1e3, y*1e3, abs(E_pro)')
    % xlabel('x / [mm]')
    % ylabel('y / [mm]')
    % shading flat
    % colorbar
    ...
    eff_s(ite) = efficiency;
    %% for  p-pol
    alpha_ana   = alpha_ana*(cos(2*theta).^2);
    
    efficiency  = 100 - 100*exp(-trapz(x, alpha_ana));
    fprintf('power scattered out = %2.1f [%%] \n', efficiency);
    eff_p(ite) = efficiency;
end
%% final figures
figure(5); clf;
plot( ...
    phis*180/pi + 90, eff_s, ...
    phis*180/pi + 90, eff_p, 'LineWidth', 2);
xlabel('diffraction angle / [deg]');
ylabel('efficiency / [%]');
legend('s-pol', 'p-pol', 'Location', 'best');
set(gca, 'FontSize', 16);